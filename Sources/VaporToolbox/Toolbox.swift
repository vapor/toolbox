    ...
    do {
      func missingVaporOutput() {
        context.console.output(
          "\("note:", style: .warning) this Swift project does not depend on Vapor.")
        context.console.output(
          key: "framework",
          value:
            "Vapor framework for this project: this Swift project does not depend on Vapor. Please ensure you are in a Vapor project directory. If you are, ensure you have built the project with `swift build`. You can create a new project with `vapor new MyProject`"
        )
      }
      ...
      case PackageResolvedV1.version:
        ...
        if let vapor = v1.object.pins.first(where: { $0.package == "vapor" }) {
          if let version = vapor.state.version {
            ...
            context.console.output(
              key: "framework", value: "Revision-\(vapor.state.revision) Unknown version")
          }
        } else {
          missingVaporOutput()
        }
      ...
      context.console.output(
        "\("note:", style: .warning) no Package.resolved file was found. Possibly not currently in a Swift package directory"
      )
      context.console.output(
        key: "framework",
        value:
          "Vapor framework for this project: no Package.resolved file found. Please ensure you are in a Vapor project directory. If you are, ensure you have built the project with `swift build`. You can create a new project with `vapor new MyProject`"
      )
    }
  }
    ...
    do {
      if let version = staticVersion {
        ...
        if let match = versionFinder.firstMatch(
          ...
          range: .init(location: 0, length: versionString.utf16.count))
        {
